...
- pick a league
- open the table in chat
- browse teams ordered by the standings
- open individual player stat cards in chat
...
3. run the bot so it serves the newest local snapshot data in chat
...
Current in-chat views:

- `View Table`
  Shows the current league table in a Telegram message, including recent form.
- `Browse Teams`
  Shows teams ordered by the standings, then players ordered by shirt number.
- player details
  Shows the selected player's latest locally refreshed stat line in chat.

The bot no longer uses CSV/PDF as the primary Telegram interaction model.
Those files are generated by the refresh pipeline for archival use and optional
manual sharing.
...
4. use Railway only as the always-on bot host
...
- League tables are now pulled from the current standings page, not from
  Matchday 1.
- The primary user experience is now in-chat browsing, not file download menus.
