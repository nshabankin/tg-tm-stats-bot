...
class Tablespider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        for team in range(1, len(teams)+1):  # iterate over every team
            item['rank'] = response.xpath(
                body + f'tr[{team}]/td[1]/text()').extract()
            item['club'] = response.xpath(
                body + f'tr[{team}]/td[3]/a/text()').extract()
            item['played'] = response.xpath(
                body + f'tr[{team}]/td[4]/text()').extract()
            item['wins'] = response.xpath(
                body + f'tr[{team}]/td[5]/text()').extract()
            item['draws'] = response.xpath(
                body + f'tr[{team}]/td[6]/text()').extract()
            item['losses'] = response.xpath(
                body + f'tr[{team}]/td[7]/text()').extract()
            item['goals'] = response.xpath(
                body + f'tr[{team}]/td[8]/text()').extract()
            item['diff'] = response.xpath(
                body + f'tr[{team}]/td[9]/text()').extract()
            item['points'] = response.xpath(
                body + f'tr[{team}]/td[10]/text()').extract()
            yield item
            items.append(item)
